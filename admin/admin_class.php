...
Class Action {
	...
	function login2(){
		...
		if($qry->num_rows > 0){
			...
			if($_SESSION['login_alumnus_id'] > 0){
				$bio = $this->db->query("SELECT * FROM bio_alumni where id = ".$_SESSION['login_alumnus_id']);
				if($bio->num_rows > 0){
	...
	function signup(){
		...
		if($save){
			...
			$save_alumni = $this->db->query("INSERT INTO bio_alumni set $data ");
			if($data){
	...
	function update_account(){
		...
		if($save){
			...
			$save_alumni = $this->db->query("UPDATE bio_alumni set $data where id = '{$_SESSION['bio']['id']}' ");
			if($data){
				foreach ($_SESSION as $key => $value) {
					unset($_SESSION[$key]);
				}
	...
	function update_alumni_acc(){
		extract($_POST);
		$update = $this->db->query("UPDATE bio_alumni set status = $status where id = $id");
		if($update)
			return 1;
	}
